...
class Lexer:
    def __init__(self, error_registry):
    ...
    def output_parsing_errors(self, errors, verbose: int):
        print(errors)
        console.print(
            f"[white on red bold]{len(errors)} parsing error{'s' if len(errors) > 1 else ''} raised.",
            highlight=False,
        )
        for error_index, found_error in enumerate(errors):
            token_index = found_error.error.token_index
            error_code = found_error.error.error_code

            if verbose > -1:
                console.print(
                    f'[[white]{error_index}[red]] {self.error_registry.get_error(error_code).message} Error Code: {error_code}',
                    style='bold red',
                    highlight=False,
                )
            if verbose > 0:
                console.print(
                    f"    {found_error.error.message}",
                    style='bold red',
                    highlight=False,
                )
            if verbose > 1:
                console.print(
                    f'    {self.error_registry.get_error(error_code).description}',
                    style='bold red',
                    highlight=False,
                )
            console.print(found_error.code)
            console.print(
                ' ' * (token_index + 3 + len(str(found_error.line_number))) + '^',
                style='bold yellow',
            )

    def lex_code(self, input_code: str, verbose: int = 0):
        processed_code = ''
        ...
        for line_number, line_content in enumerate(input_code.splitlines()):
            ...
            parsed_line = self.lex_line(line_content)
        ...
        # Output the errors
        if found_errors:
            self.output_parsing_errors(found_errors, verbose)
