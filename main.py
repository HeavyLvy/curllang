...
def parse_code(input_code: str, verbose: int = 0):
    ...
    for line_number, line_content in enumerate(input_code.splitlines()):
        ...
        if parsed_line.get('error'):
            syntax = Syntax(
                ...
                start_line=len(processed_code.splitlines()) - 5 if len(processed_code.splitlines()) > 5 else 1,
            )
            found_errors.append({'error': parsed_line['error'], 'code': syntax, 'line': line_number})
    ...
    if found_errors:
        ...
        for error_index, error in enumerate(found_errors, 1):
            ...
            if verbose > 1:
                console.print(f"    {ERROR_CODES[error['error']['code']]['description']}", style="bold red",
                              highlight=False)
...
ERROR_CODES = {
    ...
    1: {
        ...
        'description': 'Caused by inproper syntax.'
    },
    2: {
        'message': 'Invalid Syntax.',
